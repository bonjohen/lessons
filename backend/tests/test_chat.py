...
class TestChatEndpoint:
    ...
    def test_chat_no_backend(self):
        with patch("app.api._deps._generator", None), patch("app.api._deps._retriever", None):
            response = client.post("/api/chat", json={"message": "question"})
            assert response.status_code == 200
            data = response.json()
...
class TestRetrieveEndpoint:
    ...
    def test_retrieve_no_backend(self):
        with patch("app.api._deps._retriever", None), patch("app.api._deps._generator", None):
            response = client.post("/api/retrieve", json={"query": "testing"})
            assert response.status_code == 200
            data = response.json()
            assert data["chunks"] == []
